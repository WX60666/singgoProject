...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			ral = cm.align, result="style=\"", clas = cm.classes || 'ui-ellipsis', nm = cm.name, celp, acp=[];
				...
				if( $(ind).hasClass('jqgrow') ) {
					if(usedata) {
						...
						$('td[role="gridcell"]',ind).each( function(i) {
							...
							if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && nm !== 'operate') {
		...
		if(unformatFunc !== undefined && $.isFunction(unformatFunc) ) {
			...
			switch(formatType) {
				...
				case 'currency':
					...
					break;
				case 'checkbox':
					var cbv = (options.colModel.editoptions) ? options.colModel.editoptions.value.split(":") : ["true","false"];
